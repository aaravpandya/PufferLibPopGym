...
def _skip_if_wrong_env():
    ...
    if C.env_name != "popgym_noisy_position_only_pendulum":
        pytest.skip(
            "Build the target env first: "
            "PYTHON=.venv/bin/python ./build.sh popgym_noisy_position_only_pendulum --cpu"
        )
    return C
...
def test_noisy_position_only_pendulum_smoke():
...
def test_noisy_position_only_pendulum_zero_noise_on_unit_circle():
    vec = _make_vec(total_agents=8, noise_sigma=0.0)
    try:
        obs = np.ctypeslib.as_array(
            (ctypes.c_float * (vec.total_agents * vec.obs_size)).from_address(vec.obs_ptr)
        ).reshape(vec.total_agents, vec.obs_size)
...
def test_noisy_position_only_pendulum_timeout_preserves_reward():
